function [objOut_cell] = peakGrouping(Nframe, clusterGrid_cell, db_doppler_cell)

objOut_cell = cell(1, Nframe);
% frame마다 반복
for frames = 1:Nframe
 % 2D CFAR에서 detecting된 target의 Range, Doppler 인덱스 구하기
 % detected_points_2D row: range, column: doppler
 [row_2d, col_2d] = find(clusterGrid_cell{frames} ~= 0);
 ...
 for i = 1:length(row_2d)
     cellPower(i) = db_doppler_cell{frames}(row_2d(i), col_2d(i));
 end
 % 입력 detMat 구하기 
